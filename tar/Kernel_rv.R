...
for (i in 2013:2013) {
  for (j in 04:08) {
    m = i * 100 + j
    filename = paste("./" , as.character(m), "_rv_30min", sep="")
    ...
    for (rowPivot in 1:(num_of_cols )) {
      ...
      outputFileName = paste("~/Desktop/", as.character(m), "kernel_rv_plot/", as.character(m), "_", as.character(rowPivot), ".jpg", sep="")
      ...
      dev.off()
      outputmatrixname = paste("~/Desktop/", as.character(m), "kernel_rv_matrix/", as.character(m), "_", as.character(rowPivot), ".txt", sep="")
...
jpeg(outputFileName, width=1500, height=1500)
	...
	axis(1, at=1:num_of_cols, labels=tickerLabels,cex.axis=0.4)#cex:lable size  ##run k-rv-04 change to "rows"
